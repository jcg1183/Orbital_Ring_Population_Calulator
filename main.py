...
def save_results(rings):
	...
	for ring in rings:
		...
		pops_list.extend([ring.start_point.lat, ring.start_point.lon, ring.azim, ring.width])
	...
	poplines_name = str(rings[0].start_point.lon) + "_lon_poplines_" + str(int(rings[0].width/1000)) + "km"

	with open("../results/" + "poplines_" + str(int(rings[0].width/1000)) + "km" + "/" + poplines_name + ".csv", "w", newline="") as f:
		writer = csv.writer(f)
		writer.writerows(index_pops)
...
def analyze_results(rings):
	...
	df = pd.DataFrame(columns = ['Start_Lat', 'Start_Lon', 'Azim', 
								 'End_Lat', 'End_Lon',
								 'Resolution', 'Width', 'Population', 
								 'Path_Time', 'Pop_Time', 'Total_Time'])

	for ring in rings:

		df.loc[len(df)] = [ring.start_point.lat, ring.start_point.lon, 
						   ring.azim, 
						   ring.end_point.lat, ring.end_point.lon,
						   ring.resolution, ring.width, 
						   ring.population, ring.path_calculation_time, 
						   ring.population_calculation_time, 
						   (ring.path_calculation_time + ring.population_calculation_time)]
...
def run_city_file(lines):
	...
	for line in lines:
		...
		for azim in range(90, -90, -1):
			proc_args.append([start_lat, start_lon, azim, 0, 0, resolution, width, interval])
...
def run_lat2_file(lines):
	...
	for line in lines:
		...
		proc_args.append([start_lat, start_lon, -99, end_lat, end_lon, resolution, width, interval])
